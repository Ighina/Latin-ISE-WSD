...
class BertForSequenceLabeling(nn.Module):
	...
	def __init__(self, 
        ...
        self.classifier_cross_encoder = nn.Linear(bert_dim*2, num_labels)
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None, transforms=None, labels=None):
        ...
        if self.cross_encoder:
            if labels is not None:
                out = self.aggregate_pairs(all_layers, labels, transforms)
                logits = self.classifier_cross_encoder(out)
                
                loss_fct = CrossEntropyLoss()
                lloss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
    			return loss
            else:
                pass # TODO: inference (no labels, we just want the softmax scores)
    ...
    def aggregate_pairs(self, batch, labels, indices):
        new_batch = []
        new_labels = []
        for index, b in batch:
            for index2, b2 in batch:
                
                if index<index2:
                    # Extract just the target words and then concatenate them: if they represent the same meaning give a label of 1 otherwise 0
                    first_token = b[indices[index][0]:indices[index][1]].mean(axis=0)
                    second_token = b2[indices[index][0]:indices[index][1]].mean(axis=0)
                    
                    new_batch.append(toch.cat((first_token, second_token)))
                    
                    if labels[index]==labels[index2]:
        ...
        return torch.tensor(new_batch), torch.tensor(new_labels)

	def get_batches(self, sentences, max_batch, cross_encoder = False):
		...
		for sentence in sentences:
			...
			for idx, word in enumerate(sentence):
				...
				for j in range(cur,cur+len(toks)):
                    if cross_encoder and word[1]>-1:
                        ind = [cur, cur+len(toks)]
                    else:
					    ind[j]=1./len(toks)
				cur+=len(toks)
...
if __name__ == "__main__":
	...
	devN=[0.]*epochs
	testN=[0.]*epochs
	AllTest = [[0.]*epoch for _ in data]
	
	result_df = {"lemma":[], "accuracy": []}
	...
	if mode == "train":
        ...
        result_df.to_csv("results_all_lemma.csv")
