...
# 			result_df["lemma"].append(lemma)
			...
			for epoch in range(epochs):
				...
				c, t, all_cor_v=model.evaluate(dev_batched_data, dev_batched_mask, dev_batched_labels, dev_batched_transforms, metric, tagset)
				devCors[epoch]+=c
				devN[epoch]+=t

				c, t, all_cor_t=model.evaluate(test_batched_data, test_batched_mask, test_batched_labels, test_batched_transforms, metric, tagset)
				testCors[epoch]+=c
				testN[epoch]+=t
				AllTest[lemma_idx][epoch] = c/t
				cor_idx = 0
				for idx in test_ordering:
					sent = ""
					label = -100
